...
/**
 * tailor-genie-app をスタンドアロンバブリとしてビルドする設定
 *
 * ビルドコマンド:
...
export default defineConfig({
  ...
  build: {
    ...
    lib: {
      ...
      name: "TailorGenieBubly",
      fileName: () => "bubly.js",
      formats: ["iife"],
    },
    rollupOptions: {
      // 共有依存関係は外部化（ホストアプリから提供される）
      // tailor-genie-libs と tailor-genie-model はバンドルに含める（完全分離のため）
      external: (id) => {
        // tailor-genie-libs と tailor-genie-model はバンドルに含める
        if (
          id === "@bublys-org/tailor-genie-libs" ||
          id.startsWith("@bublys-org/tailor-genie-libs/") ||
          id === "@bublys-org/tailor-genie-model" ||
          id.startsWith("@bublys-org/tailor-genie-model/")
        ) {
          return false;
        }
      ...
      output: {
        globals: (id) => {
          ...
          // @mui/icons-material/Chat -> window.__BUBLYS_SHARED__.MuiIcons.Chat
