...
import { Task, type TaskState } from "@bublys-org/hotel-shift-puzzle-model";
...
type StateWithTask = RootState & { hotelShiftPuzzleTask: TaskSliceState };

const selectTaskListRaw = (state: StateWithTask) =>
  state.hotelShiftPuzzleTask?.taskList ?? [];
...
export const selectSelectedTaskId = (state: StateWithTask): string | null =>
  state.hotelShiftPuzzleTask?.selectedTaskId ?? null;
...
export const selectTaskById = (id: string) =>
  createSelector(
    [
      (state: StateWithTask) =>
        (state.hotelShiftPuzzleTask?.taskList ?? []).find((t) => t.id === id),
    ],
...
export const selectSelectedTask = createSelector(
  [
    (state: StateWithTask) => {
      const id = state.hotelShiftPuzzleTask?.selectedTaskId;
      if (!id) return undefined;
      return (state.hotelShiftPuzzleTask?.taskList ?? []).find((t) => t.id === id);
    },
  ],
  (taskState): Task | undefined =>
    taskState ? new Task(taskState) : undefined
