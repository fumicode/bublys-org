...
hotel-shift-puzzle-model（依存なし）
    ↑
hotel-shift-puzzle-libs（model + bubbles-ui + state-management）
    ↑
hotel-shift-puzzle-app（libs + bubbles-ui）
```
...
| `hotel-shift-puzzle-slice` | Member[] + selectedMemberId + ShiftPreference[] |
...
| URL パターン | 機能 |
|---|---|
| `hotel-shift-puzzle/shift-plans` | シフト案一覧（エントリー） |
| `hotel-shift-puzzle/shift-plans/:id/primitive-gantt` | セルグリッドガント |
| `hotel-shift-puzzle/shift-plans/:id/task-gantt` | タスク軸ガント |
| `hotel-shift-puzzle/shift-plans/:id/shifts/:shiftId/status` | 配置状況（統合） |
| `hotel-shift-puzzle/shift-plans/:id/shifts/:shiftId/status/members` | 配置メンバーのみ |
| `hotel-shift-puzzle/shift-plans/:id/shifts/:shiftId/status/coverage` | 充足テトリスのみ |
| `hotel-shift-puzzle/shift-plans/:id/history` | 世界線グラフ |
| `hotel-shift-puzzle/members` | 局員一覧（エントリー） |
| `hotel-shift-puzzle/members/filter` | 局員フィルター検索 |
| `hotel-shift-puzzle/members/:memberId` | 局員詳細 |
| `hotel-shift-puzzle/members/:memberId/availableShifts` | 参加可能シフト |
| `hotel-shift-puzzle/tasks` | タスク一覧（エントリー） |
| `hotel-shift-puzzle/tasks/filter` | タスクフィルター検索 |
| `hotel-shift-puzzle/tasks/:taskId` | タスク詳細 |
...
**エントリーポイントは3つ：**
1. `hotel-shift-puzzle/shift-plans` — シフト案一覧から始まる配置編集
2. `hotel-shift-puzzle/members` — 局員一覧から始まる局員管理
3. `hotel-shift-puzzle/tasks` — タスク一覧から始まるタスク管理

---

## 新機能の追加方法

### 1. ドメインモデルの変更（hotel-shift-puzzle-model）
...
ビルド確認：
```bash
cd hotel-shift-puzzle-bubly/hotel-shift-puzzle-model
npm run build
```

### 2. Redux スライスにアクションを追加（hotel-shift-puzzle-libs/src/slice/）
...
### 3. UI コンポーネントを作成（hotel-shift-puzzle-libs/src/ui/）
...
### 4. Feature コンポーネントを作成（hotel-shift-puzzle-libs/src/feature/）
...
### 5. バブルルートに登録（hotel-shift-puzzle-app/src/registration/bubbleRoutes.tsx）
...
// hotelShiftPuzzleBubbleRoutes 配列に追加
{ pattern: "hotel-shift-puzzle/my-feature/:id", type: "my-feature", Component: MyFeatureBubble },
```

### 6. ObjectView でダブルクリック展開を設定（hotel-shift-puzzle-libs/src/object-type-registration.ts）
...
`hotel-shift-puzzle-libs/src/data/` に技大祭向けのマスターデータが定義されています。
...
```
your-event-bubly/
  your-event-model/       # ← hotel-shift-puzzle-model を元にドメインを再設計
  your-event-libs/        # ← hotel-shift-puzzle-libs を元に feature/ui を実装
  your-event-app/         # ← hotel-shift-puzzle-app を元にルート登録
```

### 最小構成で始める手順

1. **`hotel-shift-puzzle-model` をコピーしてドメインを整理**
...
5. **`bubbleRoutes.tsx` を作成**
   - `hotelShiftPuzzleBubbleRoutes` と同様の配列を export
...
# テスト
npx nx test hotel-shift-puzzle-libs
```
