...
```
shift-puzzle-bubly/
  shift-puzzle-model/   # ドメイン層（純粋 TypeScript、外部依存なし）
  shift-puzzle-libs/    # 機能・UI 層（React + Redux）
  shift-puzzle-app/     # アプリ層（バブルルート登録、エントリーポイント）
```

### 依存方向

```
shift-puzzle-model（依存なし）
    ↑
shift-puzzle-libs（model + bubbles-ui + state-management）
    ↑
shift-puzzle-app（libs + bubbles-ui）
```
...
### 1. ドメインモデルの変更（shift-puzzle-model）
...
ビルド確認：
```bash
cd shift-puzzle-bubly/shift-puzzle-model
npm run build
```

### 2. Redux スライスにアクションを追加（shift-puzzle-libs/src/slice/）
...
### 3. UI コンポーネントを作成（shift-puzzle-libs/src/ui/）
...
### 4. Feature コンポーネントを作成（shift-puzzle-libs/src/feature/）
...
### 5. バブルルートに登録（shift-puzzle-app/src/registration/bubbleRoutes.tsx）
...
### 6. ObjectView でダブルクリック展開を設定（shift-puzzle-libs/src/object-type-registration.ts）
...
`shift-puzzle-libs/src/data/` に技大祭向けのマスターデータが定義されています。
...
```
your-event-bubly/
  your-event-model/       # ← shift-puzzle-model を元にドメインを再設計
  your-event-libs/        # ← shift-puzzle-libs を元に feature/ui を実装
  your-event-app/         # ← shift-puzzle-app を元にルート登録
```
...
```bash
# ドメインモデルのビルド確認
cd shift-puzzle-bubly/shift-puzzle-model && npm run build

# アプリの開発サーバー起動（bublys-os 経由）
npx nx dev bublys-os

# テスト
npx nx test shift-puzzle-libs
```
