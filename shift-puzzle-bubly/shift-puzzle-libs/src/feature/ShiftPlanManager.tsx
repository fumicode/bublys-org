...
export const ShiftPlanManager: FC<ShiftPlanManagerProps> = ({
  ...
  const handleAddPlan = () => {
    ...
    const newPlan = ShiftPlan.create(`シフト案${planNumber}`, "晴れ");
    dispatch(addShiftPlan(newPlan.state));
    setSelectedPlanId(newPlan.id);
  };
  ...
  const handleCopyPlan = (planId: string) => {
    ...
    const newPlan = ShiftPlan.create(`${sourcePlan.name}のコピー`, "晴れ");
      ...
      <div className="e-content">
        {selectedPlan ? (
          <ShiftPlanEditor
            ...
            onMemberViewClick={() => onMemberViewClick?.(selectedPlan.id)}
          />
        ) : (
