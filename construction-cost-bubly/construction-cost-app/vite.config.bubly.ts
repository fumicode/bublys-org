...
/**
 * construction-cost-app をスタンドアロンバブリとしてビルドする設定
 *
 * ビルドコマンド:
...
export default defineConfig({
  ...
  build: {
    ...
    lib: {
      ...
      name: "ConstructionCostBubly",
      fileName: () => "bubly.js",
      formats: ["iife"],
    },
    rollupOptions: {
      // 共有依存関係は外部化（ホストアプリから提供される）
      // construction-cost-libs と construction-cost-model はバンドルに含める（完全分離のため）
      external: (id) => {
        // construction-cost-libs と construction-cost-model はバンドルに含める
        if (
          id === "@bublys-org/construction-cost-libs" ||
          id.startsWith("@bublys-org/construction-cost-libs/") ||
          id === "@bublys-org/construction-cost-model" ||
          id.startsWith("@bublys-org/construction-cost-model/")
        ) {
          return false;
        }
      ...
      output: {
        globals: (id) => {
          ...
          // construction-cost-libs はバンドルに含まれるため、ここには来ない
          // MUI関連
