# construction-cost-bubly — 開発者ガイド
...
```
construction-cost-bubly/
  construction-cost-model/   # ドメイン層（純粋 TypeScript、外部依存なし）
  construction-cost-libs/    # 機能・UI 層（React + Redux）
  construction-cost-app/     # アプリ層（バブルルート登録、エントリーポイント）
```

### 依存方向

```
construction-cost-model（依存なし）
    ↑
construction-cost-libs（model + bubbles-ui + state-management + world-line-graph）
    ↑
construction-cost-app（libs + bubbles-ui + state-management）
```
...
- **バブル URL スキームは app 層に置く**（オブジェクトの正規 URL も含む）。
  libs（domain/ui/feature）に `construction-cost/...` の URL 文字列を書かない。
...
### 1. ドメインモデル（construction-cost-model/src/lib/）
新クラスを追加し、`lib/index.ts` から export する。
```bash
cd construction-cost-bubly/construction-cost-model && npm run build
```

### 2. Redux スライス（construction-cost-libs/src/slice/）
...
### 3. UI component（construction-cost-libs/src/ui/）
Redux を使わず props で受ける純粋な表示 component。`ui/index.ts` から export。

### 4. Feature component（construction-cost-libs/src/feature/）
`useAppSelector` / `useAppDispatch` で Redux と UI をつなぐ。`feature/index.ts` から export。

### 5. バブルルート（construction-cost-app/src/registration/bubbleRoutes.tsx）
`constructionCostBubbleRoutes` 配列にルートを追加。URL ビルダーは `registration/bubbleUrls.ts`
...
### 6. ObjectView ダブルクリック展開（construction-cost-libs/src/object-type-registration.ts）
```typescript
...
```bash
# ビルド
npx nx build @bublys-org/construction-cost-model
npx nx build @bublys-org/construction-cost-libs

# テスト
npx nx test @bublys-org/construction-cost-libs
