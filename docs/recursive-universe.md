...
7. root の useBubbleArrangementWorldLine: history.pushState("/universe@<新root apex>")
...
| [`bubble-ui/world-line/snapshot-url.ts`](../apps/bublys-os/app/bubble-ui/world-line/snapshot-url.ts) | `<name>@<node>` 文法の単一定義（`UNIVERSE_URL_BASE = "universe"`, `AT_PREFIX = "@"`, `buildUniverseUrl` / `readUniverseAt` ヘルパー） |
...
### B. universe ルート名 `"universe"` が依然 app 内に固定

`UNIVERSE_URL_BASE = "universe"` は [`snapshot-url.ts`](../apps/bublys-os/app/bubble-ui/world-line/snapshot-url.ts) 1ヶ所に集約済み（以前は `useUniverseWorldLine.ts` の中に埋まっていた）。1ヶ所に絞ったぶん見通しはよくなったが、それでも「世界線まわりのコードがアプリ側のルート名を知っている」という構造自体は変わっていない。
