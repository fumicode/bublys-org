...
- ネスト側 hook `useUniverseWorldLine` は `UniverseLink.snapshot` 経由で codec を受け取り、"universe" のような具体名を一切知らない

を達成。`bubbleRoutes.tsx` の universe ルートは `makeSnapshotRoute` で 1 か所だけ。

同じ仕組みで `world@<node>` / `room@<node>` 等、別の base を持つ「snapshot pointer 型ルート」を増やしても、`useUniverseWorldLine` を改変せずに使い回せる（中身が universe 以外でもよくなる、という意味で hook 名は次に見直す候補）。
