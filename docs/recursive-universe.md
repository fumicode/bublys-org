...
WorldLineGraph.grow(stateRef) → 新 apex ノード作成
   ↓ apex 変化
[rehydrate effect]
   ↓ shell.object.toJSON() を replaceBubbleViewState で Redux に流し込む
[view = apex の中身に追従]
```
...
| 役割 | hook | scope |
|---|---|---|
| root universe の commit/rehydrate + ブラウザ履歴連携 | `useBubbleViewWorldLine` | `BUBBLE_VIEW_SCOPE = "main"` |
...
7. root の useBubbleViewWorldLine: history.pushState("#wl=<新root apex>")
...
| [`bubble-ui/world-line/bubbleViewDomain.ts`](../apps/bublys-os/app/bubble-ui/world-line/bubbleViewDomain.ts) | `BubbleViewState` を world-line に乗せるための型登録 |
| [`bubble-ui/world-line/useBubbleViewWorldLine.ts`](../apps/bublys-os/app/bubble-ui/world-line/useBubbleViewWorldLine.ts) | **root** の commit/rehydrate + ブラウザ履歴コーディネータ。stale hash の self-heal、popstate→moveTo、apex→pushState を集約 |
...
| [`bubble-ui/world-line/BubbleViewWorldLineControls.tsx`](../apps/bublys-os/app/bubble-ui/world-line/BubbleViewWorldLineControls.tsx) | undo/redo ボタンと世界線 DAG ビュー（ドラッグできるフローティングパネル） |
| [`bubble-ui/world-line/BubbleViewStateInspector.tsx`](../apps/bublys-os/app/bubble-ui/world-line/BubbleViewStateInspector.tsx) | 現 view の JSON を左下に表示する開発用パネル |
...
| [`bubbles-ui/src/lib/state/bubbles-slice.ts`](../bublys-libs/bubbles-ui/src/lib/state/bubbles-slice.ts) | universeId キーで束ねた bubbleState スライス。`navigateBubble`（バブルの url 遷移）、`replaceBubbleViewState`（rehydrate 出口）など |
...
### `selectBubbleViewState` で `renderedRect` を剥がす理由
...
### A. `selectBubbleViewState` に「孤児バブル」が貯まる
...
1. `replaceBubbleViewState` の入り口で「`process.layers` に居ない bubble は捨てる」正規化を入れる
...
### C. `useBubbleViewWorldLine` の `trailRef` は本当に必要か
...
### E. `BubbleViewState` クラスは薄すぎる

`BubbleViewState` は `state: BubbleViewStateJson` をそのまま包むだけのラッパー。
