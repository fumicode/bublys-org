...
### C. `useRootArrangementWorldLine` の `trailRef` は本当に必要か

root のコーディネータは undo/redo ボタンの `canUndo / canRedo` のために**自前の訪問トレイル**を持っている（ブラウザ履歴スタックの中身は読めないので）。一方、`useUniverseArrangementWorldLine` 側は `scope.graph.canUndo / canRedo` を返している。
