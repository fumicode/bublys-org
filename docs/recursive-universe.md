...
### D. `popChildInProcess` の async listener は2経路に分かれている — **解消（listener side self-abort 採用）**

`popChildInProcess` / `joinSiblingInProcess` の listener が `await listenerApi.take(renderBubble)` で待っている間にユーザーが「戻る」を押すと、rehydrate で対象バブルが state から消えてしまう。後から遅れて発火する `renderBubble` で stale listener が起き上がり、削除済みバブルへ向けて `updateBubble` を dispatch しようとしていた（履歴トレイル汚染の原因）。

reducer ガード（[`bubbles-slice.ts`](../bublys-libs/bubbles-ui/src/lib/state/bubbles-slice.ts) で「存在しないバブルへの `updateBubble`/`renderBubble` は no-op」）でサイレントに症状は止まっていたが、根本治療として **listener 側にも self-abort** を入れた：

```ts
// bubbles-listener.ts の await から復帰した直後で：
...
- **stale 検知ロジックが listener のコードの中に明示される** → debug 時「あれ、何で commit が走ったの？」を追う時にここを見れば「stale でも諦めるはず」と分かる
- reducer ガードは**サイレントな安全網**として残す（2 重防御）

詳細なシーケンス図と他の選択肢の比較は [popchild-flow.md](popchild-flow.md) を参照。

listener が rehydrate を跨いで生きている事実そのものは変わっていないが、起き上がった瞬間に**自分が時代遅れだと気づいて諦める**ので、外部から見ると一貫性が保たれる。
...
### F. `conversation-meta` の `instanceof` 問題 — **解消**

`ConversationMeta` を「`state: { id }` を持つ薄いクラス」に格上げ
（`tailor-genie-libs/src/lib/feature/TailorGenieProvider.tsx`）。`Speaker` や
`Conversation` と同じ「Xxxx class + 内部 XxxxState (plain)」規約に揃った。

これにより `domain-registry` の登録を `class: Object` フォールバックから
`class: ConversationMeta` に直せて、`addObject(new ConversationMeta({ id }))`
と instanceof 経由で型解決できるようになった（型文字列引数の `addObject` は
