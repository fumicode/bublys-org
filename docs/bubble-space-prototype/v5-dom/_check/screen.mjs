...
//   node docs/bubble-space-prototype/v5-dom/_check/screen.mjs
...
// 1440×900 で破綻しない（右上の一覧・選択欄が画面に入る）
{
  const box = await lab.page.evaluate(() => {
    ...
    const c = document.getElementById("stage").getBoundingClientRect();   // DOM 版：canvas ではなく #stage
    return { bottom: r.bottom, right: r.right, canvasBottom: c.bottom, canvasRight: c.right };
  });
  console.log(`  右上の欄の下端 ${box.bottom.toFixed(1)} / 画面の下端 ${box.canvasBottom.toFixed(1)}`);
