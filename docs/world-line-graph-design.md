...
`grow()` した結果、世界全体の状態が **apex の 1 ステップ隣（祖先 or 直近の子）に
既にある world と一致する場合は、新しいノードを作らずそこへ移動する**。

「操作 → 間違えたと思って打ち消す → 元の状態に戻る」のように結局元へ戻ったなら、
世界線も無駄に伸ばさず、操作で一歩伸びた枝だけを残す、という仕様。

| 状況 | 挙動 |
|---|---|
| 操作 → 打ち消し → 元の状態 | 一致する**祖先**へ戻る。伸びた枝だけ残る |
| 打ち消した操作をやり直し | 一致する**直近の子**へ前進。既存の枝に合流し重複を作らない |
| 状態の変わらない no-op 保存 | apex 自身と一致 → 何も増えない |
| 一部のオブジェクトのみが変わっていて全体は違う | 一致せず通常どおり新しい world が生える |

判定は **world 全体の一致**で行う（変更した 1 オブジェクトだけ戻しても、他が変わって
いれば「元の状態」ではないため）。範囲は apex の 1 ステップ隣に限り、離れた world へ
飛ぶことはない。
...
### pinned（固定メンバー）オブジェクト

スコープに含まれるが、そのスコープ内では変更されないオブジェクト。参照として見えるが `changedRefs` には載らないため、巻き戻しの影響を受けない。

例：話者（Speaker）を pinned にして、会話の世界線の中では変更しないようにする。

**実装との対応（旧称 sealed）。** 実装では `Membership.pinned`（バブリ側の記述子）と
`CellRole.'pinned'`（世界線ビュー側）で「固定メンバー」と呼ぶ。厳密には、
世界の誕生時に参照が焼き付けられるので**起点ノードの changedRefs には載る**。
載らないのはそれ以降で、だから巻き戻しの影響を受けない。
`sealed` は実装に一度も現れなかった先行語彙なので、`pinned` に統一する。
...
- **pinned 以外のオブジェクト属性**: 同一オブジェクトが複数スコープに属する場合の扱い
