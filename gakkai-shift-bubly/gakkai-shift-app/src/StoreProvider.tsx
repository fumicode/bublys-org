...
// gakkai-shift slices（ライブラリのimport時に自動注入される）
import '@bublys-org/gakkai-shift-libs';

// gakkai-shift-app用の初期バブルを設定
setInitialBubbleUrls(['gakkai-shift/staffs', 'gakkai-shift/shift-plans']);
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  if (!storePersistorRef.current) {
    initializeApp();
    // gakkai-shift-app専用のpersistKeyを使用してOSとのlocalStorage競合を防ぐ
    storePersistorRef.current = makeStore({ persistKey: 'gakkai-shift-standalone' });
  }
