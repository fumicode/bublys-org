...
import { emptyWorld, presetView, resolveWorld } from '@bublys-org/bubble-layout';
...
export function BubbleSpace(props: BubbleSpaceProps) {
  ...
  // url は domain に入れない（「泡に url を持たせるか」は未決）。ここで id との対で持つ
  const [urls, setUrls] = useState<ReadonlyMap<BubbleId, string>>(new Map());
  ...
  const openBubble = useCallback(
    (url: string, openerId?: BubbleId | null, label?: string): BubbleId => {
      ...
      const r = openAt({
        world, viewport, openerId: openerId ?? null, newId: id,
        title: titleOf(routes, url, label), size: route.size, hue: route.hue, rules,
      });
      setWorld(r.world);
      setUrls((m) => new Map(m).set(id, url));
      setSelectedId(id);
      return id;
    },
  ...
  const api: BubbleSpaceApi = useMemo(
    () => ({ openBubble, closeBubble, urlOf: (id) => urls.get(id) ?? null, canOpen }),
