...
/**
 * 透視ビュー ── **左上へ退きながら**小さくなる 3 枚。
 * **奥ほど薄い**（泡と同じ）。ここだけ「真ん中が大きい」の語彙を使わない。
 *
 * ★ 退く向きは**実際の並びと同じ**。子の空間の消失点は「自分の箱の左上の角」なので、
 *   奥へ行くほど左上へ退く（実測：手前の札ほど右下で大きい）。
 *   前は右上へ逃げていて、絵と並びで向きが食い違っていた。
 */
const StackDepthIcon: FC = () => (
  <Svg>
    <rect x="1.2" y="1.2" width="5.4" height="5.4" rx="1.2" opacity="0.4" />
    <rect x="3.4" y="3.8" width="6.4" height="6.4" rx="1.3" opacity="0.68" />
    <rect x="5.6" y="6.6" width="7.4" height="7.4" rx="1.4" />
  </Svg>
);

/**
 * **追いかけ合う** ── 箱を変えたら並べ方が、並べ方を選んだら大きさが変わる。
 * 枠と、外へ開く矢印（動いてよい箱）。
 */
...
/**
 * **並べ方を留める** ── 箱をどう変えても並べ方は変わらない。
 * 枠は動かず、中身が右へはみ出して切れている（入らないぶんは送って見に行く）。
 */
