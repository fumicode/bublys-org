...
export const METRICS = {
  ...
  K_PERSP: 0.26,
  /**
   * 透視：焦点より**手前**の泡を、どこまで消さずに置いておくか（**刻みに対する割合**）。
   *
   * ラボは焦点の面を 1mm でも越えたら消していた（`dz < 0` で即 0）ので、
   * 手前へ繰ってきた泡が**原寸ちょうどでパッと消える**。見る側には
   * 「まだ普通の大きさなのに、もう消えた」と映る。
   * 刻みの 0.35 ぶんだけ行き過ぎても同じ面のうち、とする ── 透視は 1/(1+0.26·dz) なので、
   * 消える直前の大きさは **1.10 倍**。カメラの脇をかすめてから消える。
   *
   * ★ **刻みに対する割合**なのが肝。生の dz で決めると、刻みの細かい並べ方
   *   （重ねて置く ＝ 0.15）では何枚も手前に居残って、触った泡を覆ってしまう。
   *   割合なら「1枚ぶんの 1/3 強」がどの並べ方でも同じ意味になる。
   */
  Z_FRONT_KEEP: 0.35,
  /** root の消失点（画面中心から） */
  ROOT_VP: { x: -130, y: -165 },
