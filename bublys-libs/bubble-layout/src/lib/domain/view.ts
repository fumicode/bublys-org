...
export interface AxisView {
  ...
  /**
   * **その軸で、箱が中身に合わせて伸びるか**（既定は伸びる）。
   *
   * ④ 等間隔・詰めるの軸では、箱は中身が収まるまで伸びる ── それが既定。
   * `false` にすると**自前の大きさのまま**で、入らないぶんは箱からはみ出す（見切れる）。
   *
   * ★ 「広げる」と「見切れさせる」は**どちらも正しい**。並べ方を変えるたびに
   *   窓の大きさまで変わってほしい人と、窓は動かさず中を送って見たい人がいる
   *   ── 決めるのは見る側なので、View の軸に持たせて外から切り替えられるようにする。
   */
