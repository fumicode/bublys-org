...
export function fitFocus(
  ...
  /**
   * (1) 焦点は泡の上に居る ── ただし **0 も止まり所**。
   *
   * ★ 0 は「並びが置かれたそのまま」＝ 口のぶんを空けた（`reserve`）いちばん上。
   *   泡の場所だけで挟んでいたころは、**空けた所へ並びが戻ってきて**いた
   *   ── 札が 1 枚だと場所の幅が無い（min ＝ max ＝ 空けたぶん）ので、焦点がそこへ
   *   引き寄せられ、絵の上では**空けた 40px が丸ごと消える**（実測：CSV の一覧で
   *   いちばん上の札が ＋新規 の口へ 7px 潜り込んだ）。
   *   はみ出している側（上の `room2`）は前から 0 を含めていた。ここも同じ決まりにする。
   * ★ 足すのは**空けてあるときだけ**。空けていない並びの 0 は「中央」なので、
   *   止まり所に足すと端から先へ 1 つぶん余計に送れてしまう。
   */
  const stops = (L.view[axis].reserve ?? 0) > 0 ? [0] : [];
  v = clamp(v, Math.min(...ps, ...stops), Math.max(...ps, ...stops));               // (1)
