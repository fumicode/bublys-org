 ...
 * WorkingDay — 日付
 *
 * 「7/1」のような日を、単なる Date ではなく独自の値オブジェクトとして表現する。
 * 年・月・日を保持し、カレンダー演算が必要なときだけ内部で Date を使う。不変。
 */
...
export class WorkingDay {
  ...
  /** 一意キー・ISO日付 "2026-07-01"（辞書順でソート可能） */
  get key(): string {
  ...
  /** from..to（両端含む）を列挙する */
  static range(from: WorkingDay, to: WorkingDay): WorkingDay[] {
    const days: WorkingDay[] = [];
    let cur = from;
    // 安全のため上限を設ける（最大 366 日）
    for (let i = 0; i < 366 && cur.compareTo(to) <= 0; i++) {
      days.push(cur);
      cur = cur.next();
    }
    return days;
