# construction-shift-puzzle-bubly — 開発者ガイド
...
```
construction-shift-puzzle-bubly/
  construction-shift-puzzle-model/   # ドメイン層（純粋 TypeScript、外部依存なし）
  construction-shift-puzzle-libs/    # 機能・UI 層（React + Redux）
  construction-shift-puzzle-app/     # アプリ層（バブルルート登録、エントリーポイント）
```

### 依存方向

```
construction-shift-puzzle-model（依存なし）
    ↑
construction-shift-puzzle-libs（model + bubbles-ui + state-management + world-line-graph）
    ↑
construction-shift-puzzle-app（libs + bubbles-ui + state-management）
```
...
```
construction-shift-puzzle-model/src/
  index.ts                     # lib/index を再エクスポート
  lib/index.ts                 # ← ドメインモデルをここに追加（export {} のみ）

construction-shift-puzzle-libs/src/
...
construction-shift-puzzle-app/src/
...
- **バブル URL スキームは app 層に置く**（オブジェクトの正規 URL も含む）。
  libs（domain/ui/feature）に `construction-shift-puzzle/...` の URL 文字列を書かない。
...
- **グローバル型を origin スコープへ取り込むパターン**（テンプレート → 世界線独自コピー）：
  ...
  - 取り込む型は、id が origin 用のときだけ origin のローカル世界線へ束ねるよう `localScope` を
    宣言する（グローバル固定IDのときは `undefined`）。例（`objects/constructionObjects.tsx` の `WorkShiftSet`）:
...
### 1. ドメインモデル（construction-shift-puzzle-model/src/lib/）
新クラスを追加し、`lib/index.ts` から export する。
```bash
cd construction-shift-puzzle-bubly/construction-shift-puzzle-model && npm run build
```

### 2. Redux スライス（construction-shift-puzzle-libs/src/slice/）
...
### 3. UI component（construction-shift-puzzle-libs/src/ui/）
Redux を使わず props で受ける純粋な表示 component。`ui/index.ts` から export。

### 4. Feature component（construction-shift-puzzle-libs/src/feature/）
`useAppSelector` / `useAppDispatch` で Redux と UI をつなぐ。`feature/index.ts` から export。

### 5. バブルルート（construction-shift-puzzle-app/src/registration/bubbleRoutes.tsx）
`constructionShiftPuzzleBubbleRoutes` 配列にルートを追加。URL ビルダーは `registration/bubbleUrls.ts`
...
### 6. ObjectView ダブルクリック展開（construction-shift-puzzle-libs/src/object-type-registration.ts）
```typescript
...
```bash
# ビルド
npx nx build @bublys-org/construction-shift-puzzle-model
npx nx build @bublys-org/construction-shift-puzzle-libs

# テスト
npx nx test @bublys-org/construction-shift-puzzle-libs
