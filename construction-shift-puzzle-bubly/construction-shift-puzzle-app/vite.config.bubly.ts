...
/**
 * construction-shift-puzzle-app をスタンドアロンバブリとしてビルドする設定
 *
 * ビルドコマンド:
...
export default defineConfig({
  ...
  build: {
    ...
    lib: {
      ...
      name: "ConstructionShiftPuzzleBubly",
      fileName: () => "bubly.js",
      formats: ["iife"],
    },
    rollupOptions: {
      // 共有依存関係は外部化（ホストアプリから提供される）
      // construction-shift-puzzle-libs と construction-shift-puzzle-model はバンドルに含める（完全分離のため）
      external: (id) => {
        // construction-shift-puzzle-libs と construction-shift-puzzle-model はバンドルに含める
        if (
          id === "@bublys-org/construction-shift-puzzle-libs" ||
          id.startsWith("@bublys-org/construction-shift-puzzle-libs/") ||
          id === "@bublys-org/construction-shift-puzzle-model" ||
          id.startsWith("@bublys-org/construction-shift-puzzle-model/")
        ) {
          return false;
        }
      ...
      output: {
        globals: (id) => {
          ...
          // construction-shift-puzzle-libs はバンドルに含まれるため、ここには来ない
          // MUI関連
